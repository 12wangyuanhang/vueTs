<template>
    <div class="boxs">
        ...
        <div class="mask">
            <p>x : {{ x }} y:{{ y }} z :{{ z }}</p>
            <el-button @click="autoRotate">转动</el-button>
            <el-button @click="stopRotate">停止</el-button>

            <el-button v-if="robot.robot" @click="robotAnimate">机器人运动</el-button>
            <el-button v-if="robot.robot" @click="stopRobotAnimate">
                机器人停止
            </el-button>
...
let loadingWidth:any = ref(0); // 加载进度显示
...
const setScene = () => {
    ...
    (document.querySelector(".boxs") as any).appendChild(renderer.domElement);
};
...
/** 设置模型动画 */
const setTweens = (obj:any, newObj:any, duration = 1500) => {
...
const loadWithDraco = (module: GLTFLoader, url:string): any => {
...
const addStats = () => {
    stats = Stats();
    (document.querySelector(".boxs") as any).appendChild(stats.domElement);
};
...
const robotAnimate = () => {
    ...
    const setCircle = (num: number, upperLimit?:any, lowerLimit?:any) => {
...
const init = async () => {
    ...
    if (gltf?.scene?.children[0].name == "sole") {
        ...
        robot.armHeader = gltf.scene.children[0].children[2].children[0];
        robot.fork001 =
            gltf.scene.children[0].children[2].children[0].children[4];
        robot.pulley001 =
            gltf.scene.children[0].children[2].children[0].children[4].children[4];
        robot.tong001 =
            gltf.scene.children[0].children[2].children[0].children[4].children[4].children[3];
